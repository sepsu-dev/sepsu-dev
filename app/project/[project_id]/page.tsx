...
export default function ProjectDetailPage() {
  ...
  const staticProject = project_id ? STATIC_PROJECTS[project_id] : null;
  // Fallback: project dari DB yang belum ada di daftar statis tetap bisa dirender
  const project: ProjectData | null = staticProject ?? (project_id
    ? {
        title: project_id.replace(/-/g, "."),
        description: "",
        imageUrl: "",
        tags: [],
        demoUrl: "",
    ...
    return (
      <div className="relative min-h-screen bg-background pb-12 font-sans overflow-hidden pt-12">
        ...
        <div className="max-w-3xl mx-auto px-4 pt-28 pb-16 md:pt-36 md:pb-24 relative z-10 text-center space-y-4">
          ...
          <Link href="/#projects" className="inline-flex items-center gap-2 text-sm text-primary hover:text-primary/80 transition-colors font-medium">
          ...
          <div className="md:col-span-4 space-y-8">
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-col gap-3">
                {project.demoUrl && (
                <a href={project.demoUrl} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between p-3.5 rounded-xl bg-primary/90 hover:bg-primary text-primary-foreground text-sm font-medium shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-300 group">
                  <span className="flex items-center gap-2"><Globe className="w-4 h-4" /> Live Demo</span>
                  <ArrowUpRight className="w-4 h-4 transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
                </a>
                )}
                {project.githubUrl && (
                  <a href={project.githubUrl} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between p-3.5 rounded-xl border border-primary/20 bg-transparent hover:bg-muted/50 hover:shadow-md hover:-translate-y-0.5 text-foreground text-sm font-medium transition-all duration-300 group">
